...
void common_any_test_impl(Alg alg) {
  ...
  /*
    using T = typename Variation::type;
    std::vector<T, eve::aligned_allocator<T, 4096>> page(4096 / sizeof(T),
    T{0});
    ...
    auto run = [&]() {
      ...
      REQUIRE(alg(traits, f, before_l + 1, 1));
    };
...
void common_any_test(Alg alg) {
  ...
  common_any_test_traits_combinations<std::uint8_t>(alg);
  common_any_test_traits_combinations<std::int16_t>(alg);
  common_any_test_traits_combinations<std::uint16_t>(alg);
  common_any_test_traits_combinations<std::int32_t>(alg);
  common_any_test_traits_combinations<std::uint32_t>(alg);
  common_any_test_traits_combinations<std::int64_t>(alg);
  common_any_test_traits_combinations<std::uint64_t>(alg);
  common_any_test_traits_combinations<float>(alg);
  common_any_test_traits_combinations<double>(alg);
}
